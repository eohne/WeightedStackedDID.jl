...
"""
function stacked_did_reg(data::DataFrame;yvar::Symbol, timeID::Symbol, unitID::Symbol, cohort::Symbol, kappa_pre::Int, kappa_post::Int,x_vars::Vector{Symbol}=Symbol[],fes::Vector{Symbol}=Symbol[],cluster::Union{Nothing,Symbol}=nothing,contrasts::Dict{Symbol, DummyCoding}=Dict{Symbol, DummyCoding}(), multi_thread::Bool=true,inc_sub_exp_fes=false)
    dataset = select(data, unique(vcat(yvar,x_vars,fes,timeID,unitID,cohort)))    
    dropmissing!(dataset)
    if inc_sub_exp_fes    
        _formula=term(yvar)~sum(vcat(term(:event_time_treat),term.(x_vars),fe.(term.(vcat(fes,:treat,:event_time,:subex_unitID,:subex_etime)))))
    else
